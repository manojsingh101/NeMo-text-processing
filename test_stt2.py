...
class DateVerbalizer(GraphFst):
    def __init__(self):
        super(DateVerbalizer, self).__init__('date', 'verbalize')
        ...
        # Create a date conversion FST
        year_fst = pynini.acceptor(
            [
                (str(i), str(i)) for i in range(10)
            ]
        )
        month_fst = pynini.string_map(months)
        day_fst = pynini.string_map(days)

        # Define the pattern for a date
        date_pattern = pynini.closure(pynini.acceptor("0"), 0, 1) + year_fst + "-" + pynini.closure(pynini.acceptor("0"), 0, 1) + month_fst + "-" + pynini.closure(pynini.acceptor("0"), 0, 1) + day_fst

        # Create the verbalization FST
        verbalization_fst = year_fst + pynini.acceptor("-", " ") + month_fst + pynini.acceptor("-", ", ") + day_fst

        # Assign the date verbalizer to the FST
        self.set_start(0)
        self.set_final(0)
        self.add_arc(0, 0, pynini.epsilon, pynini.epsilon)
        self.add_arc(0, 0, date_pattern, verbalization_fst)

# Test the date verbalizer
verbalizer = DateVerbalizer()
date = "2023-06-21"
verbalized_date = pynini.compose(date, verbalizer).string()
print(verbalized_date)
